...
class BooksCoverAPI:
    ...
    def set_cover_size(self, cover_size: str = "L") -> None:
        ...
        if cover_size not in accepted_cover_sizes:
            raise ValueError(f"The informed Book Cover Size ({cover_size.upper()}) is invalid.")

        self.cover_size = cover_size.upper()

    def set_metadata_type(self, meta_type: str = "id") -> None:
        accepted_meta_types = ["ISBN", "OCLC", "LCCN", "OLID", "ID"]
        if meta_type not in accepted_meta_types:
            raise ValueError(f"The informed Book MetaID Type ({meta_type.upper()}) is invalid.")
